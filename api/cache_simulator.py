...
if __name__ == "__main__":
    ...
    cache_blocks = int(input("Cache Blocks: "))
    block_size = int(input("Block Size: "))
    read_policy = int(input("Read Policy (0 or 1): "))
    replacement_algorithm = int(input("Replacement Algorithm (0 or 1): "))
    cache_access_time = int(input("Cache Access Time: "))
    main_memory_access_time = int(input("Main Memory Access Time: "))

    cache = CacheSimulator(cache_blocks, block_size, read_policy, replacement_algorithm, cache_access_time, main_memory_access_time)
    print("--- SEQUENTIAL SEQUENCE ---")
    cache.simulate_sequential()
    print("--- MID-REPEAT BLOCKS ---")
    cache.simulate_mid_repeat()
